...
myMap  <- leaflet(data=EPAair_PM_avg) %>% 
  addTiles() %>% # tiles are background maps, default is from open street maps
...
# first filter then create a buffer
Durham_buffer <- EPAair_PM_avg_sf %>% 
  filter(Site.Name == 'Durham Armory') %>% 
  st_buffer(0.1) # geometry is in lat lon, so this is degrees
...
#Recall what the CRS is of our EPA dataset, Coordinate Reference System
...
#Check the CRS
st_crs(huc8_sf) # no EPSG code but uses aea, Albers equal something
...
## aggregate so all counties are on feature
...
leaflet() %>% # create the object
  addProviderTiles(providers$Esri.NatGeoWorldMap) %>% # basemap, follow link above to find names like Esri.NatGeoWorlMap  
...
huc2_UTM_nc <- st_intersection(huc2_utm,state_sf_utm) # clip using intersection command
mapview(huc2_UTM_nc)
```
...
mapview(huc0302_counties['ALAND'], legend = FALSE) # legend clutters the map
```
...
## will get confused when you do a left join then a right join, forgets geometry is geometry
## need to use st_to_sf
...
theSite_sfp <- st_point(c(userLng,userLat)) ## spatial feature point

#Create a simple features column from the point geometry object
theSite_sfc <- st_sfc(theSite_sfp, crs = 4326) ## spatial feature column

#Transform the mask to match the CRS of the counties dataset
## transform to UTM, same as counties
...
selCounties <- counties_sf_utm[resultMask,] ## returns that row and all columns
