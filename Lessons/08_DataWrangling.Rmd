...
colnames(NTL.phys.data.PeterPaul) # contains a comments file that may be useful
dim(NTL.phys.data.PeterPaul)
summary(NTL.phys.data.PeterPaul$comments)
class(NTL.phys.data.PeterPaul$sampledate) # r has forgotten that the date column is a date
...
summary(NTL.nutrient.data.PeterPaul$comments) # can use drop.levels() to get rid of levels that no longer correspond to a filtered dataset
...
# Save/export processed nutrient file
...
# Remove columns that are not of interest for analysis, select chooses columns
# put - before column names if you want to remove them instead of keep them
...
NTL.nutrient.data.PeterPaul.gathered <- gather(NTL.nutrient.data.PeterPaul.skinny, "nutrient", "concentration", tn_ug:po4) # gather the column names into the column "nutrient" and the values in the column "concentration"
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration)) # get rid of the rows that have na for "concentration" column
...
# Spread nutrient data into separate columns, the opposite of gather
# the column does not need to be a factor, R will recognize
# first argument made into new columns, second argument to values
...
# this example separates year, month, and day from the full date column
# gets rid of the original column
...
# more reproducible way to select the first and last dates
...
NTL.PeterPaul.summaries <- 
  ...
  filter(depth == 0) %>% # choose only surface depths
  group_by(lakename) %>%
