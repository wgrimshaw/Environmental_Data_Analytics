...
- bullet, but need an empty line before the bullet
+ bullet
* bullet
+ code output and text or notes all in the same document
+ allows knitting to a variety of formats
...
------- | ---------------
Neonicotinoids | Mortality levels of chemicals
USGS | Flow rates and stuff
LTER | Lakes in Wisconsin
...
```{r, echo = FALSE}
## nitrogen summary by lake
Nitrogen_summary <- Lake_data %>%
  group_by(lakename) %>%
  filter(!is.na(tn_ug)) %>%
  summarize(mean = mean(tn_ug), minimum = min(tn_ug), maximum = max(tn_ug),
            stdev = sd(tn_ug))

## phosphorus summary by lake
Phosphorus_summary <- Lake_data %>%
  group_by(lakename) %>%
  filter(!is.na(tp_ug)) %>%
  summarize(mean = mean(tp_ug), minimum = min(tp_ug), maximum = max(tp_ug),
            stdev = sd(tp_ug))
```
...
```{r, echo = FALSE}
kable(x = Nitrogen_summary, format = "latex")
kable(x = Phosphorus_summary, format = "latex", booktabs = T)
```
...
```{r}
ggplot(data = Lake_data) +
  geom_point(aes(x = sampledate, y = tn_ug))
```
