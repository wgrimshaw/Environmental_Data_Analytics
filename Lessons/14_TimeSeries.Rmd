...
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, 
                              format = "%m/%d/%y")
## remember some dates are in the future, but not changing here
...
PeterPaul.nutrients.surface <- 
  ...
  filter(depth == 0) %>%
  filter(!is.na(tp_ug)) # remove nas associated with total phosphorous
...
# S is cumulative change in tested value
...
# p-value indicates if test has detected a change point
# k point is the observation number
...
# add k of 79 to to the starting point, 35
...
```{r}
# Wrangle the USGS dataset for non-future points
...
ggplot(USGS.flow.data.trimmed, aes(x = datetime, y = discharge.mean)) +
  ...
  ## log axis to keep units and show many low values better
  scale_y_log10()
## there are some missing points if we look at the data
```
...
> ANSWER: linear or spline because relatively continuous data. linear more reliable for many environmental applications.
...
# Turn the interpolated dataset, list, into a proper dataframe
...
USGS.smktest
## s much lower than varS, variable dataset
summary(USGS.smktest)
## Is there a significant trend for each season?
## output shows november discharges are changing significantly
```
...
> ANSWER: There is an increasing trend, more so in winter months!