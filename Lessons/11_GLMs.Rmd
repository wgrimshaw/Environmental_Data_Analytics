...
y = alpha1 + alpha2 + epsilon

```{r}
summary(EPAair$Ozone)

# Evaluate assumption of normal distribution
shapiro.test(EPAair$Ozone) ## data do not fit a normal distribution
...
> ANWSER: We reject the null hypothesis that the mean is equal to or greater than 50 (t-test, t=-41.9, df = 1084, p<0.001). Ozone AQi values in 2017-2018 were significantly lower than 50.
...
# Format as a GLM
O3.twosample2 <- lm(EPAair$Ozone ~ EPAair$Site.Name) # Ozone by Site Name
summary(O3.twosample2)
## output as reference to intercept
```
...
O3.onesample.wilcox
## gives V statistic, rather than t statistic
O3.twosample.wilcox <- wilcox.test(EPAair$Ozone ~ EPAair$Site.Name)
O3.twosample.wilcox
## gives W statistic for two sample test
```
...
```{r}
## independently test normality for each site
...
## variance test to test homogeneity of variances of more than two data sets
bartlett.test(EPAair$PM2.5 ~ EPAair$Site.Name) # variances not significantly different from one another

# Format as a GLM
PM2.5.anova <- lm(EPAair$PM2.5 ~ EPAair$Site.Name)
summary(PM2.5.anova)

# Format as an aov, requires a post-hoc Tukey test to get pairwise comparisons
PM2.5.anova2 <- aov(EPAair$PM2.5 ~ EPAair$Site.Name)
summary(PM2.5.anova2)

# Run a post-hoc test for pairwise differences
TukeyHSD(PM2.5.anova2) ## triple oak and bryson city not siginificantly different from one another
...
> ANSWER: (ANOVA; F = 16.16, df = 1898, p < 0.001)
...
```{r}
## Again, can run two-way ANOVA as lm() or aov()
...
## * indicates multiplying all possible combinations
