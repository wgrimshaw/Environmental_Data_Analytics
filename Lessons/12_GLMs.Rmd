...
> ANSWER: Irradiance is predicted to be 487 at the water surface and decreases by 96 with each unit increase in depth (linear regression; R^2^ = 0.31, DF = 15,449, p < 0.001). For linear regression, want R-squared, df, and p value.
...
irradiancebydepth <- 
  ...
  ylim(0, 2000) + # un-comment to ignore outlier
  geom_point() 
...
```{r, fig.height = 3, fig.width = 4}
## filtering 0 data points so they can be log transformed, and there are very few of them in a large dataset
...
## log(irradiance) = 6.2 - 0.7*depth + error
...
# SE confidence interval (gray area) can also be removed
irradiancebydepth2 <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
    geom_point() +
    scale_y_log10() +
...
## TP = 6(at zero depth and zero DO so meaningless) + 1.5*depth + 0.9*DO
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit() # need to remove all nas for corrplot to work
...
```{r}
PeterPaul.naomit <- na.omit(PeterPaul.chem.nutrients) ## removed majority of points
TPAIC <- lm(data = PeterPaul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4)
step(TPAIC) # decreases the model to the most parsimonious
