...
format(today-23, format = "%A %B %dst, %Y")
format(today, format = "")
...
```{r}
# reformat the datetime column to two digit year, then month, then day
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")

# initialize a function that amends 19 or 20 to the front of the date, 19 if the year is after '18, 20 if before '18. datetime is a string.
create.early.dates <- (function(d) {
       paste0(ifelse(d > 181231,"19","20"),d)
       })
# runs the function on the datetime column
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)

# convert the datetime column to date format, reading it as four digit year, two digit month, two digit day
...
```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data) # removes entire row if NA in any column of that row
dim(USGS.flow.data)
dim(USGS.flow.data.complete)

## function complete.cases tells you which cases have complete rows
...
ggplot(USGS.flow.data.complete) +
  geom_histogram(aes(x = discharge.mean)) # instead of geom_bar for quantitative measurements
...
ggplot(USGS.flow.data.complete, aes(x = discharge.mean)) +
  ...
  scale_x_continuous(limits = c(0, 500)) # zoom into x range from 0 to 500
  
# can also name ggplots, though not done here
ggplot(USGS.flow.data.complete) +
...
```{r, fig.height = 3, fig.width = 4}
# aesthetics within geoms to show different variables
ggplot(USGS.flow.data.complete) +
...
# more low counts of minimums, more high counts of maximums

# colors the data and subdivides it by the variable gage.height.mean.approval
ggplot(USGS.flow.data.complete) +
...
```{r, fig.height = 3, fig.width = 4}
# two box plots by approval category
ggplot(USGS.flow.data.complete) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

# group boxplots by gage.height.mean, grouping every unit increase
ggplot(USGS.flow.data.complete) +
