...
read.csv("./Data/Raw/USGS_Site02085000_Flow_Raw.csv") # just prints them in the console

# Another option is to choose with your browser, but this is not as reproducible
...
str(USGS.flow.data) # gives structure information for the variable
dim(USGS.flow.data) # gives dimenions, rows by columns
length(USGS.flow.data)

head(USGS.flow.data) # first six rows of the dataframe
head(USGS.flow.data, 10)
tail(USGS.flow.data, 5) # final rows of data, called to only give final 5 rows
USGS.flow.data[30000:30005, c(3, 8, 14)] # subset of data

class(USGS.flow.data$datetime)
class(USGS.flow.data$discharge.mean)
class(USGS.flow.data$gage.height.mean)

summary(USGS.flow.data) # summary for each column
summary(USGS.flow.data$discharge.mean) # summary for one queried column
