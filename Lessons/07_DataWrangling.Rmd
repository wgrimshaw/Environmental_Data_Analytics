...
```{r, results = "hide"}
vignette("dplyr") # in depth dplyr help
```
...
```{r}
class(NTL.phys.data$lakeid) # factor means its categorical
class(NTL.phys.data$depth) # continuous numeric variable

# matrix filtering, depth =0 and all columns
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]

# dplyr filtering, same operation as above, first select data fram, then what subset we want
...
# when we subset, the row names stayed the same as in the original dataframe
...
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) # , operates as & in this case
...
NTL.phys.data.Tuesday90s <- filter(NTL.phys.data, lakename == "Tuesday Lake" &
...
```{r}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth, temperature_C) # first depth, then temp
...
# east long lake had the highest temp in 1998, though east long lake and hummingbird lake each had
...
# lake name and everything between sampledate and temperature
